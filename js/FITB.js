...
function showBottomTray() {
    ...
    tray.classList.remove('hidden'); // show whole tray
    tray.classList.remove('collapsed'); // ensure expanded
    trayCollapsed = false;
    trayVisible = true;
...
function hideBottomTray() {
    tray.classList.add('hidden'); // slide tray off screen
    tray.classList.remove('collapsed'); // reset buttons
    trayCollapsed = false;
    trayVisible = false;
}

// ✅ Toggle collapse (buttons only)
trayToggle.addEventListener('click', () => {
    ...
    function loadQuiz(quizName, folderDisplayName) {
        ...
        fetch(`/repo/js/Quizes_FITB/${quizName}.json`)
            .then(res => res.json())
            .then(quiz => {
                if (!quiz) {
